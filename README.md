...
当前状态：**第一篇第 1～5 章已完成，第二篇第 1～9 章已建立为初稿，第三篇第 1～8 章已建立为初稿，第四篇第 1～4 章已建立为初稿，第五篇第 1～7 章已建立为初稿，第六篇第 1～8 章已建立为初稿，第七篇第 1～10 章已建立为初稿，第八篇第1～7章已建立为初稿，第九篇第 1～2 章已建立为初稿**，全书当前共 60 章。第五篇的正文范围已由第 7 章“综合验证：从部署预检到运行复盘”收束，仍保持 `draft`；第 7 章提供 G0～G7 证据门、交接模板和模拟阻断示例，不代表完成现场部署。本环境仍未完成 Arduino CLI 编译、上传、Linux 实机盘点、网络查询、服务观测、资源压力演练、队列压力演练、性能压测、故障注入、安装或启用 systemd unit、Bridge 重启、配置/凭据替换、ADB/SSH/App Lab 入口验证、Router/Bridge 重连、远程写操作、Bridge 联调和硬件实机验证。章节编号按篇重置，每一篇从第 1 章重新开始。

第三篇第 1～8 章的正文范围已建立，但各章仍保持 draft；“正文范围完成”不等于 UNO Q 实机、systemd、Bridge、MCU 联调或现场部署验证完成。

第九篇第1章《项目立项与需求基线：从场景到可验收目标》和第2章《系统架构设计：从处理器边界到可验证数据流》均为 `draft`。第1章 9 项契约测试通过；第2章 9 项离线文档/逻辑示例测试通过。本次标准全仓 pytest 为 402 项测试、355 个子测试通过。第2章建立 STM32/Bridge/Linux 分层参考架构、版本化事件字段、重复/乱序及故障处理矩阵；传感器、采样周期、阈值、存储/缓存仍未决。Fig-60 与 Fig-61 Mermaid 源已登记；Fig-61 SVG 尚未渲染或目视审阅（环境中没有 `mmdc`）；UNO Q 实机与 Bridge 恢复未验证，遥测默认关闭且没有执行器路径。
...
code/第7篇_AI/第4章_推理回归与数值一致性/README.md
code/第7篇_AI/第4章_推理回归与数值一致性/comparison.json
code/第7篇_AI/第4章_推理回归与数值一致性/compare_inference.py
code/第7篇_AI/第4章_推理回归与数值一致性/test_compare_inference.py
code/第7篇_AI/第5章_端侧推理性能评估/README.md
