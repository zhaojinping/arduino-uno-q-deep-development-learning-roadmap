...
此目录提供一个纯 Python 标准库的离线证据清单检查器，将固定的八道项目级证据门汇总为 `REPORT_ONLY` 报告。UNO Q 板载 Brick、本地 LLM、DeepSeek、图像/相机、Bridge/MCU 和安全动作等 AI 分项路径须依第10章矩阵单独记录，本检查器不会自动展开或验证它们。输入中的 `CLAIMED_PASS` 是提交者给出的声明，不代表检查器读取或独立核验了底层证据；无论所有声明如何填写，程序都不会授权部署。
