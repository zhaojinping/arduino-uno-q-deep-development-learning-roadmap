...
- [`test_dataset_evaluation.py`](test_dataset_evaluation.py)：19 项标准库测试，包含跨分区组泄漏、重复 CSV 表头、畸形数据、训练/测试隔离、六位小数边界、零分母和命令行复现性。
...
逐样本 JSON 保留 `sample_id` 和 `group_id` 以便对照 CSV；这些标识未经来源认证。
判定前将距离差用 Python `round(score, 6)` 保留六位小数，JSON 的 `raw_score` 记录相同的判定值；极近门限的样本可能因舍入进入同一档。本例不是概率校准。
