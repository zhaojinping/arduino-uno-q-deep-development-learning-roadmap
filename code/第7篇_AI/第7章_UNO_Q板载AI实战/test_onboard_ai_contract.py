...
class ChapterContentContractTests(unittest.TestCase):
    ...
    def test_part_7_directory_registers_ten_chapters_and_60_total(self):
        summary = (REPOSITORY_ROOT / "SUMMARY.md").read_text(encoding="utf-8")
        self.assertEqual(len(re.findall(r"^- \[第\d+章", summary, re.MULTILINE)), 60)
    ...
    def test_image_registry_has_unique_contiguous_figure_ids_from_existing_baseline_through_61(self):
        ...
        self.assertEqual(sorted(chapter_numbers), list(range(4, 62)))
        ...
        self.assertEqual(sorted(registry_numbers), list(range(26, 62)))
