...
- [第4章 Python Bridge 结果账本与状态查询：从返回值到可验证证据](./第4章_Python_Bridge结果账本与状态查询_从返回值到可验证证据.md)

后续章节将继续覆盖一致性、错误处理和现场联调；每篇的章节编号从第 1 章重新开始。
...
第 1～4 章（共 4 章）已建立为 `draft` 初稿。第 1 章覆盖消息模型和调用边界；第 2 章覆盖有界并发、任务生命周期与取消；第 3 章覆盖连接复用、退避、旧代次隔离与请求恢复；第 4 章覆盖 SQLite 结果账本、追加式事件和 UNKNOWN 状态查询收敛。第 3、4 章均提供独立 Python 示例、测试和章节检查器。

Fig-24～Fig-27 的产物状态见[图示登记](../../images/第4篇_PythonBridge/README.md)。Fig-27 展示结果账本与状态查询闭环；图示渲染和本地代码验证与 Router/Bridge、App Lab、MCU、目标 Linux 镜像及 UNO Q 实机联调分别记录，本篇尚未完成板端验证。
