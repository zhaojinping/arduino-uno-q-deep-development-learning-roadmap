...
## Fig-51：从采样事件到服务端遥测入口

该参考流程展示载荷检查、传输和服务端处理的责任分层，并把本章离线代码与真实 IoT 路径区分开。网络中断后的缓存/重试和远程运维命令不在本章实现范围内。

<a id="fig-51-uno-q-iot-telemetry-contract"></a>

> 图示占位：图号=Fig-51；位置=本段之后；内容=传感器采样形成带身份、启动代次、序号、时间、值、单位和质量的事件，经过边缘契约门与可选 MCU/Linux 协作，再由 MQTT/HTTP 传输至服务端校验、去重和观测；来源=本书原创，平台与协议边界见参考资料索引。
...
图源：[Fig-51 Mermaid 源文件](../../diagrams/uno-q-iot-telemetry-contract.mmd)。MCU/Linux 虚线只表示一种可选应用部署边界，不表示传感器一定接 MCU，也不表示 RPC 是网络上报协议。HTTP 和 MQTT 是传输选项，不改变同一载荷契约的字段语义。图为本书原创参考架构，不是任何相关产品或标准组织发布的官方系统图；SVG 尚未渲染和目视审阅。
