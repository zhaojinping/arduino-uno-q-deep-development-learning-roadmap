...
- 先判断是否同时需要高层计算和实时控制；如果是，拆成两侧职责，再以 Bridge/RPC 连接。
- 如果不需要两侧协同，再判断是否需要确定性时序、直接 I/O 或快速安全响应；如果是，优先考虑 MCU。
- 否则，判断是否依赖 Linux 软件、网络、文件或模型；如果是，优先考虑 MPU。
- 其余任务根据数据来源、端到端延迟、故障隔离、升级方式和长期维护成本选择。
...
流程中的 MCU、MPU 和 MPU + MCU 都是可独立到达的选择结果：只有同时需要实时控制与高层计算时，才进入双侧协同分支。这张图是学习启发式规则，不替代对硬件能力、电气条件或软件接口的逐项核验。真实项目还要用目标版本和目标负载测量延迟，确认 API、引脚占用、错误处理和安全状态。
